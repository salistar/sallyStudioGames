...
export default function HomeScreen() {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: CONFIG.COLORS.background, alignItems: 'center', justifyContent: 'center', gap: 20 }}>
      ...
      <Text style={{ color: CONFIG.COLORS.muted, fontSize: 14 }}>créatif / dessin symétrique</Text>
      {highScore > 0 && <Text style={{ color: CONFIG.COLORS.warning, fontSize: 22 }}>Record points : {highScore}</Text>}
      <Pressable
        style={{ backgroundColor: CONFIG.COLORS.primary, paddingHorizontal: 48, paddingVertical: 16, borderRadius: 30 }}
        onPress={() => router.push('/game')}
      >
        <Text style={{ color: '#fff', fontSize: 20, fontWeight: '700' }}>Créer</Text>
      </Pressable>
    </View>
  );
}
