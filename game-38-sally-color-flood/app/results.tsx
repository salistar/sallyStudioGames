...
export default function ResultsScreen() {
  const { score, status } = useLocalSearchParams<{ score: string; status: string }>();
  const isVictory = status === 'victory';

  return (
    <View style={styles.container}>
      <Text style={[styles.title, { color: isVictory ? CONFIG.COLORS.success : CONFIG.COLORS.danger }]}>
        {isVictory ? 'EXTRAORDINAIRE !' : 'DOMMAGE...'}
      </Text>
